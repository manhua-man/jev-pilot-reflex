...
import { solveLocalJev } from "./local-jev.js";
const icons = {
  Braces,
  RotateCw,
  Video,
...
async function decide() {
  ...
  try {
    ...
    if (window.__USE_REMOTE_JEV__) {
      try {
        ...
        if (res.ok) {
          ...
          throw Error("Remote decide unavailable");
        }
      } catch (_err) {
        data = solveLocalJev(state);
      }
    } else {
      data = solveLocalJev(state);
    }
    ...
    tally.cost += data.cost_usd || 0;
    tally.input += data.usage?.input_tokens || 0;
    tally.output += data.usage?.output_tokens || 0;
    if (data.pricing) updateCostTooltip(data.pricing);
    tally.latencies.push(data.latency_ms || 1.5);
